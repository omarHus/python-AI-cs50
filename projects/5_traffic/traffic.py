...
def load_data(data_dir):
    ...
    labels = []
    images = []
    # Go through each dir in data_dir
    for cat in range(NUM_CATEGORIES):

        # Get directory name for each category
        directory = os.path.join(data_dir, str(cat))

        # Go through each image in directory
        for filename in os.listdir(directory):

            # Format image as numpy ndarray
            img = cv2.imread(os.path.join(directory, filename))

            # Resize img
            img = cv2.resize(img, (IMG_WIDTH, IMG_HEIGHT))
            
            # Add label and image to lists
            images.append(img)
            labels.append(cat)
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        # Finally Add Output Layer with Size NUM_CATEGORIES
